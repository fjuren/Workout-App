...
export const quickWorkouts = async (
    userId: string
) => {
    // get all workouts // TODO any rules to limit how many workouts we pull??
    const { data: quickWorkoutData, error: quickWorkoutError } = await supabase.from('workouts').select('*')
    
    if (quickWorkoutError) {
        throw new DatabaseError('Failed to fetch user quick workouts', quickWorkoutError);
    }
...
export const quickWorkoutExercise = async ( userId: string, workoutId: string ) => {
    // get specific set of exercises for the workout
    const { data: quickWorkoutExerciseData, error: quickWorkoutExerciseError } = await supabase.from('workout_exercises').select().eq('workout_id', workoutId)

    if (quickWorkoutExerciseError) {
        throw new DatabaseError('Failed to fetch workout exercise', quickWorkoutExerciseError);
    }
...
export const acceptWorkout = async (
  ...
  workoutData: any
    ...
    const { data: workout, error: workoutError } = await supabase
        ...
        type: workoutData.sessions[0].type,
    ...
    if (exercisesError) {
        ...
        await supabase.from('ai_plans').delete().eq('id', aiPlan.id);
        throw new DatabaseError('Failed to create workout exercises', exercisesError);   
     }
